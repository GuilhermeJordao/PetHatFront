...
export class SignaturePadComponent implements OnInit {
  ...
  savePad() {
    ...
    if (matchResult) {
      ...
      this.imagemService.upload(file).subscribe((data) => {
        console.log(data);
        let id_foto = (data as any).id;
        const renamedFile = new File([blob], `${id_foto}.png`, { type: contentType });
  
        localStorage.setItem(this.id, id_foto);
      });
  
    } else {
      console.error("Não foi possível extrair o tipo de mídia dos dados Base64.");
    }
